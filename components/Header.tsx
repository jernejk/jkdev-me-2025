        ...
        <div className="no-scrollbar hidden max-w-40 items-center gap-x-3 overflow-x-auto sm:flex md:max-w-80 lg:max-w-none">
          {headerNavLinks.map((link) => (
            <Link
              key={link.title}
              href={link.href}
              className="m-1 rounded-md px-2 py-1 text-sm font-medium text-gray-400 transition-colors hover:text-cyan-600 dark:text-gray-200 dark:hover:text-cyan-300"
            >
              {link.title}
            </Link>
          ))}
