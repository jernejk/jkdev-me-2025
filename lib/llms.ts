...
export function buildLlmsTxt(): string {
  ...
  const lines = [
    `# ${siteMetadata.title}`,
    '',
    '> Technical blog by Jernej Kavka (Microsoft Foundry MVP). Focus: agentic AI, Azure AI, coding-agent workflows, .NET, EF Core, performance, conference talks and practical experiments using tools such as OpenAI Codex. Prefer canonical URLs and markdown mirrors for retrieval.',
    '',
    '## Information',
    ...
    `- [Sitemap](${siteMetadata.siteUrl}/sitemap.xml): Crawl map for canonical routes.`,
    `- [RSS](${siteMetadata.siteUrl}/feed.xml): Post feed.`,
    `- [Markdown Index](${siteMetadata.siteUrl}/md): Markdown-first post listing.`,
    '',
    '## Latest posts',
    ...posts.map(
      (post) =>
        `- [${post.title}](${post.url}) (${post.date.slice(0, 10)}) | [markdown](${post.markdownUrl})`
    ),
    '',
    '## Upcoming talks',
    ...(talks.upcoming.length > 0
      ? talks.upcoming.map(
          (talk) => `- ${talk.date.slice(0, 10)} ${talk.title} ([event](${talk.url}))`
        )
      : ['- No upcoming talks currently listed.']),
    '',
    '## Optional',
    `- [About](${siteMetadata.siteUrl}/about): Biography and profile.`,
    `- [Now](${siteMetadata.siteUrl}/now): Current focus and updates.`,
    `- [GitHub](${siteMetadata.github}): Code and repositories.`,
    `- [LinkedIn](${siteMetadata.linkedin}): Professional profile.`,
    `- [YouTube](${siteMetadata.youtube}): Video talks and recordings.`,
    `- [X](${siteMetadata.x}): Social updates.`,
    '',
    '## Citation guidance',
    '- Cite canonical jkdev.me URLs.',
    '- Prefer markdown mirrors for extraction and quoting.',
  ]
...
export function buildLlmsFullTxt(): string {
  ...
  const header = [
    ...
    `- [Speaking API](${siteMetadata.siteUrl}/api/speaking)`,
    `- [LLMs JSON](${siteMetadata.siteUrl}/llms.json)`,
    ...
    '- Prefer markdown mirrors when extracting technical content.',
