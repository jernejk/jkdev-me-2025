...
### `mvp-contributions.json` ✅ Committed
...
**Features**:
- ✅ **Safe to commit** - All sensitive data removed (GDPR deletions filtered)
...
**Features**:
- ✅ **Safe to commit** - Contains only public information
...
**Features**:
- ⚠️ **Not committed** - Regenerated on demand or by GitHub Actions
...
1. **Extract MVP data** (optional, requires `.private-data/`)
   ```bash
   npm run speaking:extract-mvp      # MVP data → mvp-contributions.json
   ```

2. **Fetch live Sessionize data**
   ```bash
   npm run speaking:fetch            # Sessionize API → sessionize.json
   ```
...
4. **Build** everything in one command
   ...
   # Runs: extract-mvp → fetch → merge
   ```

## Notes

- ✅ **`mvp-contributions.json` and `jkdev-legacy.json` are committed** to git as sanitized snapshots
- ⚠️ **`sessionize.json` is gitignored** - regenerated by scripts/GitHub Actions
- 📁 **`.private-data/`** is never committed and contains sensitive source data
- 🔄 Final output is `data/speakingData.json` (also committed)
